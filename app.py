...
def open_sheet():
    ...
    if not sheet_id:
        st.error("SHEET_ID fehlt in den Streamlit Secrets.")
        st.stop()
    return gc.open_by_key(sheet_id)

def ws(sh, tab_name: str):
    return sh.worksheet(tab_name)

def ensure_tabs_and_headers(sh):
    """
    Creates tabs if missing and ensures header rows exist.
    """
    required = {
        # App-intern: "daily_inputs", "sku_levels", "config"
    ...
    for tab, headers in required.items():
        if tab not in existing:
            sh.add_worksheet(title=tab, rows=2000, cols=max(10, len(headers) + 2))
        w = ws(sh, tab)
        vals = w.row_values(1)
        if [v.strip() for v in vals[:len(headers)]] != headers:
            w.clear()
            w.update([headers])
...
def read_tab(sh, tab_name: str) -> pd.DataFrame:
    ...
    if not records:
        # if empty, return DF with header columns if present
        headers = w.row_values(1)
        return pd.DataFrame(columns=headers if headers else [])
    return pd.DataFrame(records)

def write_tab(sh, tab_name: str, df: pd.DataFrame):
    w = ws(sh, tab_name)
...
def upsert_tab(sh, tab_name: str, df_new: pd.DataFrame, key_cols: list[str]):
    df_old = read_tab(sh, tab_name)
    if df_old.empty:
    ...
    for c in key_cols:
        df_combined[c] = df_combined[c].astype(str)

    df_combined = df_combined.drop_duplicates(subset=key_cols, keep="last")
    write_tab(sh, tab_name, df_combined)
...
def get_config_value(config_df: pd.DataFrame, key: str, default):
...
def compute_plan(levels: pd.DataFrame, stock_06: pd.DataFrame, weekday: str, batch_size: int,
                 bufferA: float, bufferB: float, close_hour: int):
    ...
    # Minimal: mind. 1 Batch verfügbar um 15 Uhr, wenn sonst leer
        ...
        if pd.notna(sales_B) and float(sales_B) == 0.0:
            if (waste >= 1) or (pd.notna(rest_close) and int(rest_close) >= 1):
                issues.append((sku, "Abend ohne Umsatz",
                               f"Sales 15–{close_hour}: 0, aber Abschrift/Rest vorhanden (waste={waste}, rest_close={rest_close})."))
        ...
        if pd.notna(rest_close) and pd.notna(sales_A) and pd.notna(sales_B):
            ...
            if balance < -2:
                issues.append((sku, "Unplausible Bilanz",
                               f"(baked06+baked14) - sales - waste - rest_close = {balance:.1f}."))
...
close_hour = get_config_value(config_df, "close_hour", DEFAULT_CLOSE_HOUR)
batch_size_default = get_config_value(config_df, "batch_size", 6)
...
close_hour = st.sidebar.slider("Ladenschluss (Stunde)", 17, 23, int(close_hour), 1)
batch_size = st.sidebar.number_input("Batch/Blechgröße (Rundung)", min_value=1, max_value=48, value=int(batch_size_default), step=1)
...
if st.sidebar.button("💾 Parameter in Sheet speichern"):
    cfg_new = pd.DataFrame([{"key":"close_hour","value":int(close_hour)}, {"key":"batch_size","value":int(batch_size)}])
    upsert_tab(sh, "config", cfg_new, key_cols=["key"])
    st.sidebar.success("Gespeichert.")

if not freq_file:
    st.info("⬅️ Lade links die FREQUENZ/Verkäufe Excel hoch (für Baseline & Checks).")
    st.stop()
...
# Load levels from sheet; fallback to baseline
levels_sheet = read_tab(sh, "sku_levels")
...
def get_daily_inputs(d: date) -> pd.DataFrame:
    df = read_tab(sh, "daily_inputs")
    if df.empty:
...
def get_prev_close_stock(d: date) -> pd.DataFrame:
...
if page == "Planung (heute)":
    ...
    stock_06 = get_prev_close_stock(plan_date)
    plan = compute_plan(
        levels=levels.rename(columns={"level_a":"level_a","level_b":"level_b"}),
        stock_06=stock_06,
        weekday=weekday,
    ...
    st.info("`stock_06` kommt aus **Rest Ladenschluss von gestern** (wenn eingetragen). `stock_14_est` ist eine Schätzung aus der Planung.")
...
# ----------------------------
    ...
    existing = get_daily_inputs(entry_date)
    ...
    if st.button("💾 Speichern", type="primary"):
        ...
        upsert_tab(sh, "daily_inputs", df_new, key_cols=["date","sku"])
        ...
        current_lv = read_tab(sh, "sku_levels")
        if current_lv.empty:
        ...
        # OOS-Verdacht: Rest=0 und Sales_B << baseline => use baseline as observed
